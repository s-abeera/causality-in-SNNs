...
# for eps in epsilon:
     ...
     cnn_optimizer = tf.keras.optimizers.Adam(cnn_lr, epsilon=eps)

     core = CustomALIF(n_in = n_inputs, n_rec = rnn_units)
     core_state = core.zero_state(batch_size, tf.float32)
     rnn_optimizer = tf.keras.optimizers.Adam(rnn_lr, epsilon=eps)
     ...
     for n_e in range(n_epochs):
        ...
        dataloader = ACREDataset(batch_size, 'val')
        ...
        print('Total Loss: {}'.format(total_loss), 'Total Loss for CNN: {}'.format(total_loss_for_cnn))
